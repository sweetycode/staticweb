...
export default function Navbar({links, children}: {links: NavLinkItem[], children?: ComponentChildren}) {
        ...
        <div className="md:flex items-center max-md:w-full max-md:order-3">
            <ul className="flex max-md:flex-col md:items-center font-medium md:space-x-8 max-md:mt-4 max-md:p-4 max-md:rounded-lg max-md:border max-md:border-gray-100">
                {links.map(({label, path}) => <li>
                    <a href={path} className="block md:hover:text-blue-700 max-md:py-2 max-md:px-3 max-md:rounded max-md:hover:bg-gray-100">{label}</a>
                </li>)}
            </ul>
        </div>
