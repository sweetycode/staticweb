...
export function b64Decode(s: string): Uint8Array {
    ...
    for (let index = 0; index < roundLength; index += 4) {
        const digits = [b64FindCharCode(data.charCodeAt(index)), b64FindCharCode(data.charCodeAt(index+1)), b64FindCharCode(data.charCodeAt(index+2)), b64FindCharCode(data.charCodeAt(index+3))]
        bytes.push(
    ...
    const digits = [b64FindCharCode(data.charCodeAt(roundLength)), b64FindCharCode(data.charCodeAt(roundLength+1)), b64FindCharCode(data.charCodeAt(roundLength+2))]
    if (isNaN(digits[0])) {
        // do nothing
